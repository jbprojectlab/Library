...
const listHelper = require('./list.spec.secret')
chai.use(listHelper)
...
describe('Functional Lists', function () {
  ...
  describe('toString', function () {
    // Useful for debugging, e.g. `console.log(someList.toString())`.
    // Consider it optional, or maybe make a help ticket if you get stuck.
  ...
  describe('takeUntil', function () {
    it('returns a copy of the list if the id does not exist', function () {
      // e.g. (a b c d e).takeUntil(c) => (a' b' c' d' e')

      expect(ln4.takeUntil('fake id').length()).to.equal(4);
      expect(ln4.takeUntil('fake id')).to.not.equal(ln4);
      expect(hasAnyImmutableChanged()).to.equal(false);
    });

    it('returns a list that only contains nodes up to the node with id', function () {
      // e.g. (a b c d e).takeUntil(c) => (a' b')

      expect(ln4.takeUntil(ln2.id).length()).to.equal(2);
      expect(ln4.takeUntil(ln2.id).next.next).to.be.null;
      expect(hasAnyImmutableChanged()).to.equal(false);
    });
  });
  ...
  describe('dropUntil', function () {
    it('returns null if the id does not exist', function () {
      expect(ln1.dropUntil('fake id')).to.equal(null);
      expect(ln4.dropUntil('fake id')).to.equal(null);
      expect(hasAnyImmutableChanged()).to.equal(false);
    });

    it('returns the sublist that starts with a node with id', function () {
      // e.g. (a b c d e).dropUntil(c) => (c d e)
      for (const listNode of [ln1, ln2, ln3, ln4]) {
        expect(ln4.dropUntil(listNode.id)).to.equal(listNode);
      }
      expect(hasAnyImmutableChanged()).to.equal(false);
    });
  });
